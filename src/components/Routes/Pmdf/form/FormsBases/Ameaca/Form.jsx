...
import { listaOpcaoArmas, listaArmasBrancas, listaAmeacas } from '../../../../../Consts/Opcoes';
...
function Ameaca() {
  ...
  const { solicitante, nome, endereco, regiaoAdministrativa, referencia, telefone, ameaca, listaArmas, outraVitima, arma, narrativa } = state;

  const [open, setOpen] = useState(false);
  const [selectedVitimas, setSelectedVitimas] = useState([]);
  const [showOutraVitima, setShowOutraVitima] = useState(false);
  const [outraAmeaca, setOutraAmeaca] = useState('');
  
  const [showOutraAmeaca, setShowOutraAmeaca] = useState(false);
  ...
  const handleChange = (field, value) => {
    if (field === 'outraAmeaca') {
      setOutraAmeaca(value);
      setState(prevState => ({
        ...prevState,
        ameaca: value === '' ? ameaca : value
      }));
    } else if (field === 'ameaca') {
      setShowOutraAmeaca(value === '');
      setState(prevState => ({
        ...prevState,
        ameaca: value === '' ? outraAmeaca : value
      }));
    } else {
  ...
  useEffect(() => {
    ...
    let text = `A pessoa de nome ${nome} informa que ${solicitante === 'vítima'
      ? `está sendo ameaçada ${ameaca !== '' ? ameaca : 'de ' + outraAmeaca}`
      : `${article} ${selectedVitimasText} está sendo ameaçado ${ameaca !== '' ? ameaca : 'de ' + outraAmeaca} por ${outraVitima}`
      }${listaArmasText ? '\n' + listaArmasText : ''}
  ...
  }, [nome, solicitante, selectedVitimas, ameaca, endereco, outraVitima, regiaoAdministrativa, referencia, telefone, listaArmas, outraArma, outraAmeaca, arma])
  return (
    <>
      <Grid item xs={12} marginBottom={2} marginLeft={4}>
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} sm={10}>
            <FormLabel style={{ fontWeight: 'bold', fontSize: 18, }} id="demo-controlled-radio-buttons-group">Qual tipo de ameaça?</FormLabel>
          </Grid>
          ...
          <Grid item xs={12} sm={10}>
            <Select
              ...
              placeholder="Ameaca"
              value={ameaca}
              onChange={(e) => handleChange('ameaca', e.target.value)}
              IconComponent={KeyboardArrowDownIcon}
              variant="outlined"
            >
              {listaAmeacas
            ...
            {showOutraAmeaca && (
              <FormControl >
                <TextField
                  sx={{ width: 300 }}
                  value={outraAmeaca}
                  onChange={(e) => setOutraAmeaca(e.target.value)}
                  label="Outra ameaça"
                  variant="outlined"
                />
              </FormControl>
